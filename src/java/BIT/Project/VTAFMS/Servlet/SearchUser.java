    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (action.equals("search")) {
                if (btnAdd != null) {
                    if (btnAdd.equals("search User")) {
                        response.sendRedirect("View/User/viewUser.jsp?name=" + search.trim() + "&page1=" + page1);
            ...
            if (action.equals("user")) {
                List<User> user;
                try {
                    user = (List<User>) UserController.getAllUsers();

                    for (int i = 0; i < user.size(); i++) {
                        out.print(user.get(i).getUser_firstName().concat(":")
                                + user.get(i).getUser_lastName().concat("!"));
